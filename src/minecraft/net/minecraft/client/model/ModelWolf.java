...
public class ModelWolf extends ModelBase {
    ...
    public void render(Entity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor) {
        ...
        if (this.isChild) {
            float scale = 2.0F;
            GL11.glPushMatrix();
            GL11.glTranslatef(0.0F, 5.0F * scaleFactor, 2.0F * scaleFactor);
            this.wolfHeadMain.renderWithRotation(scaleFactor);
            GL11.glPopMatrix();
            GL11.glPushMatrix();
            GL11.glScalef(1.0F / scale, 1.0F / scale, 1.0F / scale);
            GL11.glTranslatef(0.0F, 24.0F * scaleFactor, 0.0F);
            this.wolfBody.render(scaleFactor);
            this.wolfLeg1.render(scaleFactor);
            this.wolfLeg2.render(scaleFactor);
            this.wolfLeg3.render(scaleFactor);
            this.wolfLeg4.render(scaleFactor);
            this.wolfTail.renderWithRotation(scaleFactor);
            this.wolfMane.render(scaleFactor);
            GL11.glPopMatrix();
        } else {
            this.wolfHeadMain.renderWithRotation(scaleFactor);
            this.wolfBody.render(scaleFactor);
