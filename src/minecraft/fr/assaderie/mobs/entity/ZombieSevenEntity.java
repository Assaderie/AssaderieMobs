...
public class ZombieSevenEntity extends EntityZombie {

    private ZombieConfig config;
    private EntityLiving targetEntity = null; // cible d’attaque générale
    private int ticksNearEntity = 0;

    public ZombieSevenEntity(World world) {
        super(world);

        // Configuration de l'entité zombie
        if (!AllEntityConfig.enableZombieConfig) {
            ...
            config = ConfigZombieManager.getZombieConfig("ZombieSeven");
        ...
        this.texture = TexturesZombie.TEXTURES_ZOMBIE_SEVEN;
    ...
    protected int getDropItemId() {
        return (config != null ? config.dropEnabled : AllEntityConfig.dropEnabled) ? config.dropItemId : -1;
    }
